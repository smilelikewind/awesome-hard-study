...
> **数据源规范**：参见 `references/financial-data.md`。所有财务数据必须来自两个独立来源，误差>1%须标记。
...
数据收集完成后，**必须调用 `references/financial_rigor.py` 对关键数据进行程序化验证**，杜绝LLM心算误差。
...
```bash
python3 references/financial_rigor.py verify-market-cap \
  --price {股价} --shares {总股本} --reported {报告市值} --currency {币种}
```

Step 2 — 关键数据多源交叉验证：
```bash
python3 references/financial_rigor.py cross-validate \
...
```bash
python3 references/financial_rigor.py verify-valuation \
...
```bash
python3 references/financial_rigor.py three-scenario \
...
```bash
python3 references/report_audit.py extract \
  --report <报告文件路径>
```
输出 JSON 模板，每项含 `fetched_value`（待填）。

**Step 2 — 取数核验：**
对清单中每个数据点，按 `references/financial-data.md` 规范从可靠信源取数
...
**Step 3 — 输出判决：**
```bash
python3 references/report_audit.py verdict \
  --results '<填好的JSON>' \
  --report <报告文件名>
```
